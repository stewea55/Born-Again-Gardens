...
- **Tables (exact Supabase names):** hero_images, plant_catalog, profiles, user_preference, sponsors_public, sponsors_section_config, sponsors_private, guests, transaction, harvest_quantity, resources, volunteers, shop_catalog.
...
- **Admin dashboard (`/admin`):** Tabbed admin UI now supports CRUD editing for plant_catalog, guests, profiles, user_preference, transaction, resources, shop_catalog, sponsors_public, and volunteers. Only users with `profiles.role = 'admin'` can see the hamburger link or access the page; non-admins who open `/admin` directly are redirected to `/`. **Date:** 2026-03-02
- **Upcoming events content model:** Volunteer upcoming events are stored in `resources` rows with `page = 'volunteer'` and `resource_type = 'upcoming_event'`; title/details map to `resource_name` and `image_url`.
...
- **Volunteer upcoming events rendering:** Volunteer page now renders a clickable bullet list from the admin-managed upcoming event rows.
- **Date:** 2026-03-02
