 ...
 * For guest checkout: guestId set, user_id null; pass guestName and customerEmail.
 * For logged-in: userId set, guest_id null; pass userName and customerEmail.
 */
 ...
 * Updates transaction status by Stripe session id (used by webhook and by return-page session-status).
 * Idempotent: if transaction is already finalized (paid/sold_out), skips work.
 */
export async function finalizeCheckoutByStripeId({ stripeId, status }) {
  ...
  const { data: txn } = await supabase
    .from("transaction")
    .select("id, status, guest_id, user_id, flow_type, checkout_context")
    .eq("stripe_id", stripeId)
    .single();

  if (!txn) return;

  const alreadyFinalized = txn.status === "paid" || txn.status === "sold_out";
